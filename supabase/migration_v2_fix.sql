-- ============================================================
-- MIGRAÇÃO v2 (versão corrigida) — rode no Supabase SQL Editor
-- Execute tudo de uma vez. Se quiser rodar em partes, cada
-- bloco com "-- PARTE X" pode ser executado separadamente.
-- ============================================================

-- PARTE 1 — TABELAS NOVAS
-- ============================================================
...
-- PARTE 4 — FUNÇÃO is_admin() E TRIGGER (trigger com fallback seguro)
-- ============================================================
...
-- Trigger em auth.users (pode falhar se permissões insuficientes — ignorar o erro e continuar)
do $$
begin
...
exception when others then
  raise notice 'Trigger on_auth_user_created não criado (% — %); crie manualmente se necessário.', sqlstate, sqlerrm;
end $$;

-- PARTE 5 — RLS NAS NOVAS TABELAS
-- ============================================================
...
-- PARTE 6 — REMOVER POLÍTICAS ANTIGAS (authenticated = admin era errado)
-- ============================================================
...
-- PARTE 9 — SEED: HORÁRIOS DE AULA
-- ============================================================

insert into public.class_schedules
  (modality_name, day_of_week, start_time, end_time, target_audience, display_order)
select * from (values
  ('Karatê Infantil',             1, '17:00'::time, '18:00'::time, 'infantil', 10),
  ('Karatê Infantil',             3, '17:00'::time, '18:00'::time, 'infantil', 11),
  ('Karatê Infantil',             5, '17:00'::time, '18:00'::time, 'infantil', 12),
  ('Karatê para Jovens',          1, '18:00'::time, '19:00'::time, 'jovens',   20),
  ('Karatê para Jovens',          3, '18:00'::time, '19:00'::time, 'jovens',   21),
  ('Karatê para Jovens',          5, '18:00'::time, '19:00'::time, 'jovens',   22),
  ('Karatê Adulto',               1, '19:00'::time, '20:30'::time, 'adultos',  30),
  ('Karatê Adulto',               3, '19:00'::time, '20:30'::time, 'adultos',  31),
  ('Karatê Adulto',               5, '19:00'::time, '20:30'::time, 'adultos',  32),
  ('Karatê Adulto',               6, '08:00'::time, '10:00'::time, 'adultos',  33),
  ('Treinamento para Competição', 2, '18:00'::time, '20:00'::time, 'todos',    40),
  ('Treinamento para Competição', 4, '18:00'::time, '20:00'::time, 'todos',    41),
  ('Treinamento para Competição', 6, '10:00'::time, '12:00'::time, 'todos',    42)
) as v(modality_name, day_of_week, start_time, end_time, target_audience, display_order)
